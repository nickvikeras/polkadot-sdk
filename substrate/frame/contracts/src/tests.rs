...
use crate::{
	...
	wasm::{Memory, ReturnErrorCode as RuntimeReturnCode},
	weights::WeightInfo,
	...
	fn native_dependency_deposit_works() {
		...
		for code in [Code::Upload(wasm.clone()), Code::Existing(code_hash)] {
			ExtBuilder::default().build().execute_with(|| {
				...
				Contracts::upload_code(RuntimeOrigin::signed(ALICE), dummy_wasm.clone(), None)
					.unwrap();
	...
	fn account_reentrance_count_works() {
		...
		ExtBuilder::default().existential_deposit(100).build().execute_with(|| {
			...
			let another_contract_addr =
				builder::bare_instantiate(Code::Upload(wasm_reentrance_count))
					.value(300_000)
					.build_and_unwrap_account_id();
