...
pub use crate::wasm::runtime::{MappedMemory, ReturnData, TrapReason};
...
use crate::{
	...
	ExecError, HoldReason, Pallet, PristineCode, Weight, LOG_TARGET,
};
use alloc::vec::Vec;
...
pub struct PreparedCall<'a, E: Ext + 'a> {
	...
	pub fn call(mut self) -> ExecResult {
		let exec_result = loop {
			...
			if let Some(exec_result) =
				self.runtime.handle_interrupt(interrupt, &mut self.instance, self.api_version)
			{
				break exec_result
			}
		};
...
impl<T: Config> WasmBlob<T> {
	pub fn prepare_call<'a, E: Ext<T = T>>(
		self,
		mut runtime: Runtime<'a, E>,
		entry_point: ExportedFunction,
		api_version: ApiVersion,
	) -> Result<PreparedCall<'a, E>, ExecError> {
		...
		let gas_limit_polkavm: polkavm::Gas = runtime
			.ext()
			.gas_meter_mut()
			.gas_left()
			.ref_time()
			.checked_div(T::Schedule::get().ref_time_by_fuel())
			.ok_or_else(|| Error::<T>::InvalidSchedule)?
			.try_into()
		...
		Ok(PreparedCall { instance, runtime, api_version })
	}
}
...
impl<T: Config> Executable<T> for WasmBlob<T> {
	...
	fn execute<E: Ext<T = T>>(
		...
		let api_version = if <E::T as Config>::UnsafeUnstableInterface::get() {
			...
			// TODO: read api version from CodeInfo
			ApiVersion::Versioned(0)
		};
		let prepared_call =
