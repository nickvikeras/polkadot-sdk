...
use syn::{punctuated::Punctuated, spanned::Spanned, token::Comma, FnArg, Ident};
...
impl EnvDef {
	pub fn try_from(item: syn::ItemMod) -> syn::Result<Self> {
		...
		let host_funcs = items
			...
			//TODO: create syscall number by hashing the name
			.enumerate()
			.map(|(idx, item)| HostFn::try_from(item, idx as u32))
...
impl HostFn {
	pub fn try_from(mut item: syn::ItemFn, syscall_number: u32) -> syn::Result<Self> {
		let err = |span, msg| {
			...
			syn::Type::Path(tp) => {
				...
				match &result.arguments {
					syn::PathArguments::AngleBracketed(group) => {
						...
						Ok(Self { item, api_version, name, returns, cfg, syscall_number })
					},
					_ => Err(err(span, &msg)),
				}
			},
...
fn expand_functions(def: &EnvDef) -> TokenStream2 {
	let impls = def.host_funcs.iter().map(|f| {
		...
		let syscall_number = &f.syscall_number;
		...
		quote! {
			#cfg
			#syscall_number if __is_available__(#api_version) => {
	...
	quote! {
		// Write gas from  polkavm into pallet-contracts before entering the host function.
		...
		let result = (|| match __syscall_number__ {
			#( #impls )*
			_ => Err(TrapReason::SupervisorError(Error::<E::T>::CodeRejected.into()))
		})();

		// Write gas from pallet-contracts into polkavm after leaving the host function.
...
fn expand_bench_functions(def: &EnvDef) -> TokenStream2 {
	let impls = def.host_funcs.iter().map(|f| {
		...
		quote! {
			#cfg
			pub fn #name<M: Memory<E::T>>(&mut self, memory: &mut M, #(#params),*) #output {
				#body
			}
		}
	});
